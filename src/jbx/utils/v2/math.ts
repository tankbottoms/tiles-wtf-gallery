...
import { fromWad, percentToPermyriad } from '$jbx/utils/formatNumber';
...
export const splitPercentFrom = (percentage: number): BigNumber => {
	return percentage ? BigNumber.from((percentage * SPLITS_TOTAL_PERCENT) / 100) : BigNumber.from(0);
};
 ...
 */
export const amountAddFee = (amountWad?: string, feePerBillion?: BigNumber): string | undefined => {
	if (!feePerBillion || !amountWad) return;
